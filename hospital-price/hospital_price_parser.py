...
class HospitalDataStandardizer:
        ...
        for index, row in self.data.iterrows():
            for column in row.index:
                if self.is_payer_column(column):
                    standardized_entry = {
                        'SourceName': source_file_name, #self.get_source_name(row),#self.file_name,
                        'Code': self.clean_code(self.get_code(row)),#self.get_code(row),#self.clean_code(row.get('Billing Code', '')),
                        'CodeType': self.determine_code_type(self.get_code(row)),
                        'Description':  self.get_description(row),
                        'RevCode': self.get_rev_code(row), #row.get('Revenue Code', ''),
    ...
    def get_code(self, row):
        for column in ['Billing_Code','HCPCs', 'CPT/HCPCs']:
            if column in row and pd.notna(row[column]):
                return row[column]
                #return self.clean_code(row[column])
        return ''
    ...
    def determine_code_type(self, code):
        if not isinstance(code, str):
            code = str(code)
        if 'MS-DRG' in code:
            return 'MS-DRG'
        ...
        if 'HCPCS' in code or 'CPT' in code:
            return 'HCPCS_CPT'
        if len(code)>0: #explicitly map HCPCS-CPT code
            return 'HCPCS_CPT'  
        return ''
    
    # def get_source_name(self, row):
    #     for column in ['File_Date', 'Facility']:
    #         if column in row and pd.notna(row[column]):
    #             return row[column]
    #     return ''
    
    def get_description(self, row):
        for column in ['Code Description', 'Supply Name', 'Medication Name', 'Service_Description']:
            if column in row and pd.notna(row[column]):
                return row[column]
        return ''
    ...
    def clean_payer(self, payer):
        if 'Discounted_Cash_Price' in payer:
            return 'Cash Rate'
        elif 'Gross Charge' in payer or 'Gross_Charge' in payer:
            return 'List Price'
        else:
