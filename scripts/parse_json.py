...
# Extracting provider data
            ...
            for negotiated_rates in json_data['in_network'][y]['negotiated_rates']:
                for neg_data in negotiated_rates['negotiated_prices']:
                    in_network_dict = {
                        ...
                        #'service_code': [a if len(neg_data['service_code'])>1 else neg_data['service_code'] for a in neg_data['service_code']], 
    ...
    #print(df2.loc[(df2['billing_code'] == '20552') & (df2['provider_references'] == 5)])

    df = pd.merge(df1[['reporting_entity_name','npi','provider_group_id']]
                  ,df2[['name','billing_code_type','billing_code','negotiation_arrangement','negotiated_prices','negotiated_type','expiration_date','service_code','provider_references']]
                  ,how='inner'
    ...
    df.to_csv(outputdirectory + '\\combineddata.csv')
